...
const pickAnswer = (d: any): string => {
  ...
  return d.output || d.text || d.response || d.answer || (d.data && (d.data.output || d.data.text)) || "";
};

const mount = () => {
  const cfg = window.PB_CHAT_WIDGET_CONFIG;
  if (!cfg?.webhookUrl) {
    console.error(
      "[pb-chat-widget] Missing config. Add window.PB_CHAT_WIDGET_CONFIG = { webhookUrl: 'https://...' } before loading the script."
    );
    return;
  }
  ...
  // Host in light DOM + Shadow inside (изоляция от Тильды)
  ...
  style.textContent = `
    ...
    button {
      width:42px;height:42px;border-radius:999px;border:none;background:${primary};
  ...
  const toggle = () => (win.classList.contains("open") ? closeWin() : open());
